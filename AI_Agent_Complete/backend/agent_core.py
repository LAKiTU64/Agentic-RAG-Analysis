    ...
    async def _agent_rag_qa(self, message: str) -> str:
        ...
        if retrieved_contexts:
            ...
            for i, res in enumerate(retrieved_contexts):
                ...
                # 使用更具区分度的标记 ⁅⁅REF:x⁆⁆，防止与 [11] 混淆，由于这种括号极为罕见，所以不容易出错
                context_parts.append(
                    f"========== 片段 ⁅⁅REF:{index}⁆⁆ (文件名: {filename}) ==========\n内容：{content}\n"
                )
        ...
        prompt = f"""
            ...
            1. **引用格式**：每当你使用资料中的数据或结论时，**必须**在句子末尾加上引用编号 `⁅⁅REF:x⁆⁆`。
            2. **编号来源**：`x` 必须对应资料开头的 `========== 片段 ⁅⁅REF:x⁆⁆` 中的数字。
            3. **禁止混淆**：资料内容中可能包含 "[NSYS-KERNEL-001]" 或 "[]" 类似的编号，这些是**内容**，不是**引用编号**。只有形如**⁅⁅REF:x⁆⁆" 形式，才是引用编号。**
