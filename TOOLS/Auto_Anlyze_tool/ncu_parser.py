...
class NCUParser:
    ...
    def _parse_ncu_rep(self) -> None:
        ...
        # 生成输出文件名
        csv_file = self.input_file.with_suffix('.csv')
    ...
    def _parse_ncu_rep_json(self) -> None:
        ...
        json_file = self.input_file.with_suffix('.json')
...
{'-'*40}
• 分析kernel数量: {len(self.parser.kernels)}
• 平均SM效率: {gpu_stats.get('average_sm_efficiency', 'N/A'):.1f}% (如果有数据)
• 效率低于50%的kernel数: {gpu_stats.get('kernels_below_50_percent', 'N/A')}
...
🔥 GPU 利用率分析
{'-'*40}
• 平均SM效率: {stats.get('average_sm_efficiency', 0):.1f}%
• 最高SM效率: {stats.get('max_sm_efficiency', 0):.1f}%
• 最低SM效率: {stats.get('min_sm_efficiency', 0):.1f}%
